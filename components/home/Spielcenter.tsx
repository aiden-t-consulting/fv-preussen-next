      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {entries.map((e) => (
            ...
            >
              <td className="py-2.5 font-mono text-xs text-gray-500">{e.position}</td>
              <td
                className={cn(
                  "py-2.5 font-medium",
                  e.isCurrentTeam ? "text-[#039139]" : "text-white"
                )}
              >
                {e.team}
              </td>
              <td className="px-2 py-2.5 text-right text-gray-400">{e.played}</td>
              <td className="px-2 py-2.5 text-right text-gray-400">{e.won}</td>
              <td className="px-2 py-2.5 text-right text-gray-400">{e.drawn}</td>
              <td className="px-2 py-2.5 text-right text-gray-400">{e.lost}</td>
...
export function Spielcenter({
  ...
  return (
    <section className="bg-[#15171c] py-20 lg:py-24">
        ...
        <div className="flex flex-col lg:flex-row lg:gap-16">
          ...
          <div className="mb-10 shrink-0 lg:mb-0 lg:w-72">
            ...
            <p className="mb-6 text-sm leading-relaxed text-gray-400">
              Alle wichtigen Spielinformationen auf Basis offizieller Daten — direkt von
              FUSSBALL.DE.
            </p>
            <Link
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="rounded-[20px] bg-[#1e1d2b] p-6">
              {activeTab === "Nächste Spiele" &&
                (upcoming.length > 0 ? (
                  upcoming.slice(0, 5).map((m) => <MatchRow key={m.id} match={m} />)
                ) : (
                  <p className="text-sm text-gray-500">Keine Spiele geplant.</p>
                ))}
              ...
              {activeTab === "Tabelle" &&
                (table.length > 0 ? (
                  ...
                  <p className="text-sm text-gray-500">Keine Tabellendaten verfügbar.</p>
                ))}
            </div>
          </div>
        </div>
